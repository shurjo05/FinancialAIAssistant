...
def _make_tools(db: Session) -> list:
    """Build LLM-facing tool callables bound to this DB session.

    Signatures use simple typed args with defaults so the SDK can introspect
    them; docstrings become the tool descriptions the model reads.
    """
    ...
    def get_spending_by_category(category: str = "", start_date: str = "", end_date: str = "") -> dict:
        ...
        return tools.get_spending_by_category(db, category or None, start_date or None, end_date or None)
    ...
    def get_total(kind: str = "spending", start_date: str = "", end_date: str = "") -> dict:
        ...
        return tools.get_total(db, kind, start_date or None, end_date or None)

    def top_merchants(n: int = 5, start_date: str = "", end_date: str = "") -> dict:
        """The top N merchants by total spend over an optional date range."""
        return tools.top_merchants(db, n, start_date or None, end_date or None)
...
def gemini_answer(db: Session, question: str) -> dict:
    """Answer via Gemini with automatic function-calling."""
